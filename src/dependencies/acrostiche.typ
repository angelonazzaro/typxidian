  ...
  context {
    ...
    if acr in acronyms {
      ...
      if type(defs) == str {
        ...
        if defs.len() == 0 {
          // The user could have provided an empty array, unlikely but possible.
          panic(
        ...
        if plural {
          if "long-pl" in defs {
            ...
            panic(
              "The dictionary of definitions supplied for the key "
                + acr
                + " does not contain a plural definition with the key 'long-pl'.",
            )
          }
        } else {
          if "long" in defs {
            ...
            panic(
              "The dictionary of definitions supplied for the key "
                + acr
                + " does not contain a cingular definition with the key 'long'.",
            )
          }
        }
      } else {
        panic(
          "Definitions should be a string, an array, or a dictionary. Definition of "
            + acr
            + " is of type: "
            + repr(type(defs)),
        )
